...
namespace Aim2Pro.TrackCreator
{
    public class TrackLab : EditorWindow
    {
        ...
        static Transform EnsureRow(GameObject track,int row,bool createIfMissing=true){
            var name=$"Row_{row}"; var tr=track.transform.Find(name);
            if(!tr && createIfMissing){ var go=new GameObject(name); tr=go.transform; tr.parent=track.transform; }
            return tr;
        }
        ...
        public static void BuildGrid(int rows,int width,float tileSize,float thickness){
            ...
            for(int r=1;r<=rows;r++){
                var row=EnsureRow(track,r,true); row.localPosition=new Vector3(0f,0f,(r-1)*tileSize);
        ...
        public static void OffsetRowsY(int start,int end,float meters){
            ...
            for(int r=start;r<=end;r++){
                ...
                for(int i=0;i<row.childCount;i++){
                    var t=row.GetChild(i); var lp=t.localPosition; lp.y+=meters; t.localPosition=lp;
                }
            }
        ...
        public static void AppendStraight(float L,float step,int width,float tile,float thick){
            ...
            var track=EnsureTrack(); int maxRow=0;
            ...
            for(int s=1;s<=steps;s++){
                int r=maxRow+s;
                var row=EnsureRow(track,r,true); row.localPosition=new Vector3(0f,0f,(r-1)*tile);
                for(int c=1;c<=width;c++){
