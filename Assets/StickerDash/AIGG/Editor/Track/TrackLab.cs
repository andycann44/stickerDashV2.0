...
namespace Aim2Pro.TrackCreator
{
    public class TrackLab : EditorWindow
    {
        ...
        static Transform EnsureRow(GameObject track, int row, bool createIfMissing=true){ var name=$"Row_{row}"; var tr=track.transform.Find(name); if(!tr && createIfMissing){ var go=new GameObject(name); tr=go.transform; tr.parent=track.transform; } return tr; }
        ...
        public static void OffsetRowsX(int start,int end,float meters){
            ...
            for (int r=start;r<=end;r++){
                var row=EnsureRow(track,r,false); if(!row) continue;
                var p=row.localPosition; p.x+=meters; row.localPosition=p;
            }
        ...
        public static void OffsetRowsY(int start,int end,float meters){
            ...
            for (int r=start;r<=end;r++){
                ...
                for (int i=0;i<row.childCount;i++){
                    ...
                    var lp=t.localPosition; lp.y+=meters; t.localPosition=lp;
                }
            }
        ...
        public static void StraightenRows(int start,int end){
            ...
            for (int r=start+1;r<=end;r++){
                ...
                var p=row.localPosition; p.x=prev.localPosition.x; row.localPosition=p;

                int count=Math.Min(prev.childCount,row.childCount);
                for (int i=0;i<count;i++){
                    var prevTile=prev.GetChild(i);
                    var rowTile =row.GetChild(i);
                    var lp=rowTile.localPosition; lp.y=prevTile.localPosition.y; rowTile.localPosition=lp;
                }
            }
        ...
        public static void AppendStraight(float L,float step,int width,float tile,float thick){
            ...
            for (int i=0;i<track.transform.childCount;i++){
                ...
                if (ch.name.StartsWith("Row_") && int.TryParse(ch.name.Substring(4), out int n) && n>maxRow) maxRow=n;
            }
            for (int s=1;s<=steps;s++){
                int r=maxRow+s;
                var row=EnsureRow(track,r,true);
                row.localPosition=new Vector3(0f,0f,(r-1)*tile);
                for (int c=1;c<=width;c++){
                    var t=GameObject.CreatePrimitive(PrimitiveType.Cube);
                    t.name=$"Tile_r{r}_c{c}";
                    t.transform.parent=row;
                    t.transform.localScale=new Vector3(tile,thick,tile);
                    t.transform.localPosition=new Vector3((c-1)*tile, thick*0.5f, 0f);
                }
            }
            Debug.Log($"[TrackLab] AppendStraight: +{steps} rows (~{L:0.##}m).");
        }
