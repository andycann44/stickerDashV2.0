...
namespace Aim2Pro.AIGG.Track
{
    ...
    public class NLTrackBuilderPro : EditorWindow
    {
        ...
        Spec ParseNL(string text, bool autoStraight)
        {
            ...
            // row gaps
            foreach (Match m in Regex.Matches(t, @"\bgaps?(?:\s*of)?\s*(\d+)\s*%\b", RegexOptions.IgnoreCase))
                spec.cmds.Add($"P50|AddGaps:{GetInt(m, 1, 0)}");

            // tile gaps
            foreach (Match m in Regex.Matches(t, @"\btile\s*gaps?(?:\s*of)?\s*(\d+)\s*%\b", RegexOptions.IgnoreCase))
