...
E="$ROOT/Assets/StickerDash/AIGG/Editor/Track"
mkdir -p "$E" "$ROOT/StickerDash_Status/Plans" "$ROOT/StickerDash_Status/Notes"

cat > "$E/CanonicalPasteWindow.cs" <<'CS'
#if UNITY_EDITOR
using UnityEditor;
using UnityEngine;
using System;
...
# Notes update
NOTES="$ROOT/StickerDash_Status/Notes/SD2_WorkingNotes.md"
{
  echo ""
  echo "## $(date +%Y-%m-%d' '%H:%M:%S) — Canonical Paste window added"
  echo "- Window → Aim2Pro → Track Creator → Canonical Paste…"
  echo "- Validate, Dry Run, Apply. Save/Load .sdplan. Creates Restore Point via menu."
  echo "- Last plan at StickerDash_Status/LastCanonical.plan"
} >> "$NOTES"

echo "Installed: Assets/StickerDash/AIGG/Editor/Track/CanonicalPasteWindow.cs"
echo "Open Unity and look under Window → Aim2Pro → Track Creator → Canonical Paste…"
